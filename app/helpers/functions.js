...
exports.setPageData = function setPageData(req, result) {
  ...
  if(result && result['ID']) {
    ...
    page_data.image_src = result.featured && result.featured.full ? result.featured.full : (result.featured && typeof result.featured === 'string') ? result.featured : (config.meta.image_src.startsWith('http') ? config.meta.image_src : config.domain + config.meta.image_src);
...
exports.shortcodify = function shortcodify(prefix, lang_preurl, data, body, req_params, basepath, cb) {
  ...
  shortcode.add('avnode', function(buf, opts) {
    if (opts.view === "performances" || opts.view === "performances_inverse" || opts.view === "performances_inverse_nofloat") {
    ...
    if (opts.view === "calendar") {
      var html = jade.renderFile(__dirname+'/../views/_partials/shortcodify/calendar.pug', {basepath:basepath, prefix: prefix, lang_preurl: lang_preurl, opts: opts, req_params:req_params, result:data, body:body});
    }
    return html;
  });
